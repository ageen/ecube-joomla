...
if(count($list)>0){?>
<div id="btcontentslider<?php echo $module->id; ?>" style="display:none;margin:0 auto 15px auto;width:<?php echo $moduleWidthWrapper;?>" class="bt-cs<?php echo $moduleclass_sfx? ' bt-cs'.$params->get('moduleclass_sfx'):'';?>">
		...
		<?php foreach( $list as $i => $row ): ?>
			<div class="bt-row <?php if($i==0) echo 'bt-row-first'; else if($i==count($list)-1) echo 'bt-row-last' ?>"  style="width:<?php echo $itemWidth;?>%" >
				...
				<?php if($show_category_name_as_link) : ?>
					...
					<?php endif; ?>
					<?php if( $row->thumbnail && $align_image == "center" ): ?>
					<div class="bt-center ih-item square colored effect11 top_to_bottom">
					<a target="<?php echo $openTarget; ?>"
						...
						<div class="info">
                        	...
                        	<div class="description"><?php echo $row->introtext; ?></div>
                      	</div>
					</a>
					</div>
					<?php endif ; ?>
